...
def ask_next_question_with_ack(intent_name, session_attributes, current_question_index, user_answer=None, expected_answer=None):
    ...
    if user_answer:
        # Acknowledge both user input and expected answer
        ack_text = f"You said: {user_answer}"
        if expected_answer:
        ...
        if invocation_source == "DialogCodeHook":
            ...
            if intent_name == "WelcomeIntent":
                ...
                if next_index < len(questions):
                    return ask_next_question_with_ack(
                        intent_name,
                        session_attributes,
                        next_index,
                        user_answer=user_answer,
                        expected_answer=previous_question.get("answer")
                    )
                else:
